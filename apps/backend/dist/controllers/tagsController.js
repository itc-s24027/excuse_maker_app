import { createTagSchema, tagIdSchema, excuseTagSchema, } from "../validators/tagSchemas.js";
...
// 言い訳にタグを追加
export async function addTagToExcuse(req, res) {
    try {
...
// 言い訳からタグを削除
export async function removeTagFromExcuse(req, res) {
    try {
...
// 言い訳のタグ一覧取得
export async function getExcuseTags(req, res) {
    try {
        const excuseId = req.params.excuseId;
        if (!excuseId) {
...
// タグに関連する言い訳を取得
export async function getExcusesByTag(req, res) {
    try {
        const tagId = req.params.tagId;
        if (!tagId) {
